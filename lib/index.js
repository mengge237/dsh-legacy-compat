...
let VERBOSE = process.env.DSH_LEGACY_COMPAT_VERBOSE === '1'
function runtimeVerbose() { return VERBOSE }
function setRuntimeVerbose(v) { VERBOSE = v }
...
function say(ctx, level, ...args) {
  ...
  if (runtimeVerbose() || level === 'warn' || level === 'error') {
...
export function apply(ctx) {
  ...
  try {
    ...
    for (const target of candidates) {
      ...
      else if (runtimeVerbose()) say(ctx, 'info', '会话日志自检通过：' + r.scanned + ' 个正常')
      break
    }
  } catch (err) {
  ...
  // 设置分栏（Settings 里能看到本插件；web 等提供 settings 服务的环境生效，其它环境静默跳过）
  try {
    if (typeof ctx.inject === 'function') {
      ctx.inject(['settings'], (sctx) => {
        registerSettingsSection(ctx, sctx).catch((err) => {
          say(ctx, 'info', '设置分栏注册跳过：' + (err?.message || err))
        })
      })
    }
  } catch { /* 静默：无 settings 服务的环境不需要分栏 */ }
}

async function registerSettingsSection(ctx, sctx) {
  const zodMod = await import('zod')
  const zod = zodMod?.z ?? zodMod?.default?.z
  if (!zod || typeof sctx?.settings?.installSection !== 'function') return
  const Config = zod.object({
    verbose: zod.boolean().default(false).describe('详细日志（等同环境变量 DSH_LEGACY_COMPAT_VERBOSE=1；打开后重启前的启动日志也可见）'),
    autoScan: zod.boolean().default(true).describe('启动自检：把损坏的会话日志移入 ~/.dsh/dsh-legacy-compat-quarantine/（不删除，避免 dsh web 起不来）')
  })
  const ns = 'dsh-legacy-compat'
  let source = () => ({ verbose: runtimeVerbose(), autoScan: true })
  sctx.settings.installSection(ctx, ns, Config, { verbose: runtimeVerbose(), autoScan: true }, {
    setSource(next) { source = next },
    onChange() {
      try {
        const v = source()
        if (v && typeof v === 'object') setRuntimeVerbose(v.verbose === true)
        say(ctx, 'info', '设置已更新（verbose=' + (runtimeVerbose() ? '开' : '关') + '）')
      } catch { /* 静默 */ }
    }
  })
  say(ctx, 'info', '已注册设置分栏：Settings → ' + ns + '（含 verbose 开关；卸载：node bin/uninstall.mjs 或 dsh plugin --profile web remove dsh-legacy-compat）')
}
